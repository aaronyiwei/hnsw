...
std::vector<int> HNSWGraph::SearchLayer(float* q, int ep, int ef, int lc, float* mean, float* qmean) {
...
std::vector<int> HNSWGraph::KNNSearch(float* q, int K, float* mean, float* qmean) {
...
void HNSWGraph::Insert(LVQData& q, float* mean) {
    ...
    int ep = enter_node_;
    float  decom_vec[130],decom_vec2[130],d[130];
    DecompressTo(d,q,mean);
    for (int i = maxLyer; i > l; i--) ep = SearchLayer(d, ep, 1, i, mean, mean)[0];
    for (int i = std::min(l, maxLyer); i >= 0; i--) {
        int MM = l == 0 ? MMax0_ : MMax_;
        std::vector<int> neighbors = SearchLayer(d, ep, ef_construction_, i, mean, mean);
