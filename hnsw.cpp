...
double Dist(float *point, float *other) {
    double result = 0.0;
    for (int i = 0; i < 128; i++)
        result += (point[i] - other[i]) * (point[i] - other[i]);
    return result;
}
...
double Dist2(LVQData &v1, LVQData &v2) {
    ...
    return norm2sq_scalesq_1 + norm2sq_scalesq_2 + beta * beta * dim - 2 * scale1 * scale2 * c1c2_ip +
           2 * beta * norm1_scale_1 - 2 * beta * norm1_scale_2;
}
std::vector<int> HNSWGraph::SearchLayer(LVQData &q, int ep, int ef, int lc, float *mean) {
    ...
    while (!candidates.empty()) {
        ...
        for (int ed : layer_edgeLists_[lc][nid]) {
            if (is_visited.find(ed) != is_visited.end())
                continue;
...
std::vector<int> HNSWGraph::KNNSearch(LVQData &q, int K, float *mean) {
    ...
    for (int l = maxLyer; l >= 1; l--)
        ep = SearchLayer(q, ep, 1, l, mean)[0];
...
void HNSWGraph::Insert(LVQData &q, float *mean) {
    ...
    for (int i = maxLyer; i > l; i--)
        ep = SearchLayer(q, ep, 1, i, mean)[0];
    for (int i = std::min(l, maxLyer); i >= 0; i--) {
        ...
        std::vector<int> selected_neighbors =
            std::vector<int>(neighbors.begin(), neighbors.begin() + std::min(int(neighbors.size()), M_));
        ...
        for (int n : selected_neighbors) {
            if (layer_edgeLists_[i][n].size() > MM) {
                ...
                for (int d = 0; d < std::min(int(dist_pairs.size()), MM); d++)
                    layer_edgeLists_[i][n].push_back(dist_pairs[d].second);
            }
        }
        ep = selected_neighbors[0];
    }
    if (l == layer_edgeLists_.size() - 1)
        enter_node_ = nid;
}